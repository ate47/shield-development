...
namespace mods {
	...
	namespace {
		...
		struct scriptparsetree
		{
					...
					case gsc_custom::gsic_field_type::GSIC_FIELD_DETOUR:
					{
						...
						for (size_t j = 0; j < detour_count; j++)
						{
							...
							logger::write(logger::LOG_TYPE_DEBUG, std::format(
								"read detour {:x} : namespace_{:x}<script_{:x}>::function_{:x} / offset=0x{:x}+0x{:x}",
			...
			{
				if ((data.length() < sizeof(game::GSC_OBJ) + sizeof(acts::debug_scripting::MAGIC)) 
					|| *reinterpret_cast<decltype(acts::debug_scripting::MAGIC)*>(&data[sizeof(game::GSC_OBJ)]) != acts::debug_scripting::MAGIC)
				{
					return true; // too small to be a debug file
				}
				acts::debug_scripting::GSC_ACTS_DEBUG& dbg = *reinterpret_cast<acts::debug_scripting::GSC_ACTS_DEBUG*>(data.data() + sizeof(game::GSC_OBJ));
				...
				if (dbg.has_feature(acts::debug_scripting::ADF_STRING))
				{
				...
				if (dbg.has_feature(acts::debug_scripting::ADF_DETOUR))
				{
					for (acts::debug_scripting::GSC_ACTS_DETOUR* detours = dbg.get_detours(ptr); detours != dbg.get_detours_end(ptr); detours++)
					{
						gsc_custom::gsic_detour& detour = gsic.detours.emplace_back();
				...
				{
					if (dbg.has_feature(acts::debug_scripting::ADF_DEVBLOCK_BEGIN))
					{
